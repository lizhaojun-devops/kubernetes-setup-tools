...
do
  ...
  #因为不是内网的镜像,所以这里去掉
  #ssh root@${node} "curl http://172.19.30.116/mirror/.help/centos_install.sh | bash" 
  #ssh root@${node} "curl http://172.19.30.116/mirror/.help/epel_install.sh | bash" 
  ssh root@${node} "yum -y install yum-cron ntpdate chrony jq curl sysstat wget openssl openssh sshpass vim htop iotop iftop nload lsof"
  ...
  ssh root@${node} "ntpdate -u ntp1.aliyun.com; clock -w; systemctl disable ntpd; systemctl stop ntpd;"
