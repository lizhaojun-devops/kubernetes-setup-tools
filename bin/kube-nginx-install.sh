...
stream {
    upstream backend {
        ...
        server $ETCD_NODE1:6443        max_fails=3 fail_timeout=30s;
        server $ETCD_NODE2:6443        max_fails=3 fail_timeout=30s;
        server $ETCD_NODE3:6443        max_fails=3 fail_timeout=30s;
    }
    server {
        listen *:8443;
