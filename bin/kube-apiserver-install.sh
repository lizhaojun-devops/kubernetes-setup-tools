...
echo "检查k8s集群信息"
source ../conf/environment.conf
ssh root@${OPS_HOST} "source /etc/profile; kubectl cluster-info"
ssh root@${OPS_HOST} "source /etc/profile; kubectl get all --all-namespaces"
ssh root@${OPS_HOST} "source /etc/profile; kubectl get componentstatuses"
sleep 5s
ssh root@${OPS_HOST} "source /etc/profile; kubectl create clusterrolebinding kube-apiserver:kubelet-apis --clusterrole=system:kubelet-api-admin --user kubernetes"
